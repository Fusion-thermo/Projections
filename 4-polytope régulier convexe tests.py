...
class point:
	...
	def projections(self):
		#3D
		coos=np.array([[self.x],[self.y],[self.z],[self.w]])
		coeff=1/(float(lumiere_w_var.get()) - self.w)
...
def affichage_objet(objet):
	...
	for dots in objet.arretes:
		...
		if largeur_ligne<1:
			...
			largeur_ligne=2
		#print(origine_x + dots[0].y2d* unite,origine_y - dots[0].z2d* unite,origine_x + dots[1].y2d* unite,origine_y - dots[1].z2d* unite,largeur_ligne)
...
def revolution_xz():
	recursif = fenetre.after(10,revolution_xz)
...
def revolution_xy():
	recursif = fenetre.after(10,revolution_xy)
	global objet, revolution, revolution_angle_init
	if revolution==0:
		revolution+=1
...
def revolution_zw():
	recursif = fenetre.after(10,revolution_zw)
	global objet, revolution, revolution_angle_init
	if revolution==0:
		revolution+=1
...
def revolution_w():
	recursif = fenetre.after(10,revolution_w)
	global objet, revolution, revolution_angle_init
	if revolution==0:
		revolution+=1
...
Rxz=StringVar()
Rxz.set(0)
angle_Rxz=Scale(fenetre,  orient='horizontal',  from_=0,  to=360,  resolution=1,  tickinterval=120,  label='Rotation XZ',  variable=Rxz,  command=rotation_Rxz)
angle_Rxz.pack()
